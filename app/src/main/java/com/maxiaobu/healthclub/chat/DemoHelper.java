...
public class DemoHelper {
    ...
    public static boolean isFistLaunch=true;
...
//        options.setGCMNumber("324169311137");
        ...
        options.setMipushConfig("2882303761517525233", "5361752551233");
...
//        }
     ...
     * @return
     */
    private boolean isGroupMessageNotifer(EMMessage message){
        Log.d(TAG, message.getChatType().toString());
        if (!message.getChatType().equals(EMMessage.ChatType.GroupChat) )
            return true;
        Log.d(TAG, message.getTo());
        Set<String> stringSet = SPUtils.getStringSet(com.maxiaobu.healthclub.common.Constant.GROUP_ID);
        if (stringSet.contains(message.getTo())) {
            if(SPUtils.getInt(com.maxiaobu.healthclub.common.Constant.NOTIFY,1) == 1){
                return true;
            }else {
                return false;
            }
        ...
        messageListener = new EMMessageListener() {
            ...
            public void onMessageReceived(List<EMMessage> messages) {
                for (EMMessage message : messages) {
                    ...
                    if (!isFistLaunch&&isGroupMessageNotifer(message)&&!mIsFirstLogin) {
            ...
            public void onCmdMessageReceived(List<EMMessage> messages) {
                for (EMMessage message : messages) {
                    ...
                    String pretCode = message.getStringAttribute("pretCode", "-1");
                    if (action.equals("popads")) {
                        Intent intent = new Intent("android.intent.action.MY_BROADCAST");
                        if (!pretCode.equals("-1")) {
                            intent.putExtra("pretCode", pretCode);
